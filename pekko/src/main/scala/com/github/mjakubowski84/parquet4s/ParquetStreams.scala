...
/** Holds factory of Pekko Streams sources and sinks that allow reading from and writing to Parquet files.
  */
object ParquetStreams {

  /** Creates a [[org.apache.pekko.stream.scaladsl.Source]] that reads Parquet data from the specified path. If there are multiple
  ...
  /** Creates a [[org.apache.pekko.stream.scaladsl.Sink]] that writes Parquet data to single file at the specified path (including
