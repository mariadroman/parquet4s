...
import org.apache.pekko.actor.{Actor, ActorRef, Cancellable, CoordinatedShutdown, Props, Scheduler}
...
private class FluctuatingSchedulerActor(action: () => Unit) extends Actor {
  ...
  def scheduling(delay: FiniteDuration, direction: Direction): Receive = {
    case ScheduleNext =>
      ...
      val step = (delay.toMillis * rate).toLong.millis
